 ...
 */
class N7_Golf_Club_Elementor_Control_Choice extends \Elementor\Base_Data_Control {
	...
	public function enqueue() {
		wp_enqueue_script( 'n7-golf-club-elementor-choice-control', n7_golf_club_get_file_url( 'plugins/elementor/params/choice/choice-control.js'), array('jquery'), null, true );
	}
	...
	public function content_template() {
		...
		<div class="elementor-control-field">
			...
			<div class="elementor-control-input-wrapper">
				<div class="n7_golf_club_param_choice">
					<input type="hidden" id="<?php echo esc_attr($control_uid); ?>"
							data-setting="{{ data.name }}"
							class="{{ data.name }} {{ data.type }}_field"
							value="{{ data.controlValue }}" />
					<div class="n7_golf_club_list_choice"><#
						_.each( data.options, function( params, slug ) {
							print( ( params.new_row !== undefined
										? '<span class="n7_golf_club_list_choice_rows_separator"></span>'
										: ''
										)
									+ '<span tabindex="0" class="n7_golf_club_list_choice_item'
												+ ( ( slug == data.controlValue && '' !== data.controlValue ) || ( 'inherit' == slug && '' === data.controlValue ) ? ' n7_golf_club_list_active' : '' )
												+ '"'
										+ ' data-choice="' + slug + '"'
										+ ( params.description ? ' title="' + params.description + '"' : '' )
									+ '>'
										+ '<span class="n7_golf_club_list_choice_item_icon">'
											+ '<img src="' + ( params.icon.indexOf( '//' ) == -1 ? '<?php echo esc_url( trailingslashit( get_template_directory_uri() ) ); ?>' : '' ) + params.icon + '" alt="' + params.title + '">'
										+ '</span>'
										+ '<span class="n7_golf_club_list_choice_item_title">'
											+ params.title
										+ '</span>'
									+ '</span>');
						} );
