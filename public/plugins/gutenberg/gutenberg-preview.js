...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			n7_golf_club_create_observer( 'n7-golf-club-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				n7_golf_club_gutenberg_editor_init();
			} );
		} else {
		 	n7_golf_club_gutenberg_editor_init();
		}
	...
	function n7_golf_club_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'n7_golf_club_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'n7_golf_club_inited' ) ) {
	...
	// Init on page load
	function n7_golf_club_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = n7_golf_club_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + N7_GOLF_CLUB_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + N7_GOLF_CLUB_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in N7_GOLF_CLUB_STORAGE['override_classes'] ) {
				$obj.addClass( N7_GOLF_CLUB_STORAGE['override_classes'][i].replace( '%s', N7_GOLF_CLUB_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( n7_golf_club_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && N7_GOLF_CLUB_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'n7_golf_club_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			n7_golf_club_remove_observer( 'n7-golf-club-check-editor-styles-wrapper' );
			n7_golf_club_create_observer( 'n7-golf-club-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'n7_golf_club_inited' );
	}
} );
