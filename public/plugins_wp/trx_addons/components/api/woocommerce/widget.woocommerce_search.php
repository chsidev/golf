...
// Return list of expanded filters
...
// Add attributes from a current category to the filters list
...
class trx_addons_widget_woocommerce_search extends TRX_Addons_Widget {
	...
	function update($new_instance, $instance) {
		...
		$instance['apply'] = ! empty( $new_instance['apply'] ) ? 1 : 0;
		$instance['ajax'] = ! empty( $new_instance['ajax'] ) ? 1 : 0;
		$instance['force_checkboxes'] = ! empty( $new_instance['force_checkboxes'] ) ? 1 : 0;
		$instance['show_selected'] = ! empty( $new_instance['show_selected'] ) ? 1 : 0;
		$instance['show_counters'] = ! empty( $new_instance['show_counters'] ) ? 1 : 0;
		$instance['expanded'] = (int) $new_instance['expanded'];
		$instance['autofilters'] = ! empty( $new_instance['autofilters'] ) ? 1 : 0;
		for ($i=1; $i<=$this->search_fields; $i++) {
...
// Count products number with specified rating in current result set
...
// Count product number by attribute in the specified category
...
// Add widget's specific vars to the JS storage
