...
if (!function_exists('trx_addons_woocommerce_attrib_show_single_product')) {
	...
	function trx_addons_woocommerce_attrib_show_single_product( $html, $args ){
		...
		if ( in_array( $type, array( 'image', 'color', 'button' ) ) ) {
			...
			if ($output) {
				$html = str_replace('<select ', '<select class="trx_addons_attrib_'.esc_html($type).'" style="display:none !important;" ', $html);
...
// Show specified attributes in the product list
...
// Return a thumb size for the product list
