...
if ( ! function_exists( 'n7_golf_club_customizer_get_css' ) ) {

	function n7_golf_club_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts        = n7_golf_club_customizer_add_theme_fonts( $fonts );
			$rez          = array();
...
.n7_golf_club_tabs .n7_golf_club_tabs_titles li a {
...
CSS;
			$rez          = apply_filters( 'n7_golf_club_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			$vars = n7_golf_club_customizer_add_theme_vars( $vars );

			$rez         = array();
			$rez['vars'] = '';
			$rez         = apply_filters( 'n7_golf_club_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars'] = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( n7_golf_club_get_sorted_schemes() ) : array( $scheme );
			if ( count( $schemes ) > 0 ) {
				$rez = array();
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = n7_golf_club_get_scheme_colors( $s );
					}

					// Make theme-specific colors and tints
					$colors         = n7_golf_club_customizer_add_theme_colors( $colors );
					$rez['colors']  = '';
					$rez            = apply_filters(
						'n7_golf_club_filter_get_css', $rez, array(
							'colors' => $colors,
							'scheme' => $s,
						)
					);
		...
		return apply_filters( 'n7_golf_club_filter_prepare_css', $css_str, $remove_spaces );
	}
}
