...
?><div id="post-<?php the_ID(); ?>" <?php post_class( 'related_item post_format_' . esc_attr( $n7_golf_club_post_format ) ); ?> data-post-id="<?php the_ID(); ?>">
	...
	n7_golf_club_show_post_featured(
		array(
			'thumb_size'    => apply_filters( 'n7_golf_club_filter_related_thumb_size', n7_golf_club_get_thumb_size( (int) n7_golf_club_get_theme_option( 'related_posts' ) == 1 ? 'big' : 'med' ) ),
		)
	);
	?>
	<div class="post_header entry-header">
		...
		if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
			?>
			<div class="post_meta">
				<a href="<?php echo esc_url( $n7_golf_club_link ); ?>" class="post_meta_item post_date"><?php echo wp_kses_data( n7_golf_club_get_date() ); ?></a>
			</div>
			<?php
		}
		?>
