	...
	foreach( $template_parts as $k=>$v ) {
		...
		if ( substr( $k, 0, 4 ) == 'meta') {
			...
			if ($args['template_list'] == 'hover') {
				$image = get_the_post_thumbnail_url(get_the_ID(), n7_golf_club_get_thumb_size('big'));
			}
			...
			if (false && $args['type'] == 'list' && $args['template_list'] == 'hover_2') {
				...
				if(!empty($title)) {
					...
					echo n7_golf_club_strwords($title, 11);
					n7_golf_club_show_layout((!empty($post_link) ? '</a>' : '') . '</h6>');
				}
			} else {
